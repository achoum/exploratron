...
uint8_t kBackgroundColor = 0;
uint8_t kWhiteTextColor = 255;

uint8_t kColorEnumToRGB[(int)eColor::_NUM_COLOR][3] = {
    {kWhiteTextColor, kWhiteTextColor, kWhiteTextColor},  // WHITE
    {255, 0, 0},                                          // RED,
    {0, 0, 255},                                          // BLUE,
    {0, 255, 0},                                          // GREEN,
    {100, 100, 100},                                      // GRAY,
...
void Terminal::Initialize() {
  ...
  SDL_SetRenderDrawColor(renderer_, kBackgroundColor, kBackgroundColor,
                         kBackgroundColor, 0);
  ...
  SDL_Surface *icon = IMG_Load("exploratron/assets/graphic/icon.png");
  CHECK(icon);
...
void Terminal::CreateFontBitmap() {
  ...
  for (int i = 0; i < 128; i++) {
    ...
    SDL_SetSurfaceBlendMode(character_surface, SDL_BLENDMODE_NONE);
...
void Terminal::DrawTitleBar(int x, int y, int w, std::string text,
                            eColor color) {
  const int size = (text.size() + 1) / 2;
