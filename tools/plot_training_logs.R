...
ma <- function(x, n = 500){filter(x, rep(1 / n, n), sides = 2)}

ap = function (plot,name) {
  path = paste0("training_logs/",name,".csv")
  d = read.csv(path)
  d$smoothed_max_fitness = ma(d$max_fitness)
  d$smoothed_median_fitness = ma(d$median_fitness)
  
  plot = plot +
    geom_path(data=d, aes(x=generation,y=smoothed_max_fitness, color=name))
